...
const UUID_MIGRATION_KEY = "trungvos:indexeddb-uuid-migration-v1";
...
export function useFileSystem(
  ...
  const formatFileSystem = useCallback(async () => {
    try {
      ...
      localStorage.removeItem("trungvos:file-size-timestamp-sync-v1");
  ...
  useEffect(() => {
    const syncFileSizesAndTimestamps = async () => {
      ...
      const syncKey = "trungvos:file-size-timestamp-sync-v1";
